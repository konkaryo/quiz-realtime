...
function Lives({ lives, total }: { lives: number; total: number }) {
  ...
  return (
    <div className="flex justify-center gap-2 mt-3 text-[20px]">
      {full}{empty}
    </div>
  );
}
...
export default function RoomPage() {
  ...
  // --- Barre de temps inversée (scaleX: 1 → 0) en CSS, GPU-friendly ---
  ...
  const resetTimerBar = (dur: number) => {
    ...
    el.style.transform = "scaleX(1)"; // plein au départ (inverse)
    // démarre l’anim sur la frame suivante
    requestAnimationFrame(() => {
      el.style.transition = `transform ${dur}ms linear`;
      el.style.transform = "scaleX(0)"; // se vide vers 0
    });
  };
  const stopTimerBar = () => {
    const el = barRef.current;
  ...
  const backgroundLayer =
    "fixed inset-0 z-0 bg-[radial-gradient(1200px_800px_at_20%_10%,#1f1b42_0%,transparent_60%),radial-gradient(900px_600px_at_80%_30%,#3b0c50_0%,transparent_55%),linear-gradient(180deg,#0a0814_0%,#170e24_40%,#0c0717_100%)]";
  const grainLayer =
  ...
  return (
    <>
      ...
      <div className="relative z-10 text-white mx-auto w-full max-w-[1200px] px-4 min-h-[calc(100dvh-64px)] pt-6 font-[system-ui]">
        {/* Bandeau haut : room + message + barre temps inversée + compteur */}
        <div className="grid items-end gap-4 grid-cols-1 md:grid-cols-[minmax(0,1.1fr)_minmax(0,2fr)_minmax(0,1fr)]">
          ...
          {/* Col centre : barre inversée + label */}
          <div className="min-w-0">
            <div className="h-[8px] rounded-full bg-white/18 overflow-hidden shadow-[inset_0_1px_2px_rgba(0,0,0,.35)]">
              <div
                ref={barRef}
                className="h-full bg-gradient-to-r from-[#ffffff] to-[#ffd7f9]"
          ...
          {/* Col droite : timer sec + leaderboard badge */}
          <div className="min-w-0">
        ...
        {/* Corps principal : 1 → 3 colonnes */}
        <div className="mt-4 grid gap-6 items-start grid-cols-1 md:grid-cols-[minmax(0,1.1fr)_minmax(0,2fr)_minmax(0,1fr)]">
          {/* Col gauche (placeholder évolutif / stats rapides) */}
          <aside className="min-w-0">
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-3 shadow-[0_8px_24px_rgba(0,0,0,.25)]">
              <div className="text-sm opacity-80">Statut</div>
              <div className="mt-1 text-base">{phase === "between" ? "Next game starting…" :
          ...
          <section className="min-w-0">
            {phase === "final" ? (
              <div className="rounded-2xl border border-white/10 bg-white text-slate-900 p-5 shadow-[0_12px_30px_rgba(0,0,0,.35)]">
              ...
              <div>
                {question && (
                  <>
                    {/* Carte question */}
                    <div className="rounded-[18px] border border-white/25 bg-black/75 px-5 py-3 backdrop-blur-md shadow-[0_12px_24px_rgba(0,0,0,.35)]">
                      <div className="font-medium leading-snug tracking-[0.2px] text-[18px]">{question.text}</div>
                    </div>

                    {/* Image cadrée (aucune déformation) */}
                    {question.img && (() => {
                            ...
                            return (
                              <button
                                ...
                                className={[
                                  ...
                                  isOk ? "bg-emerald-500/20" : isSel ? "bg-blue-500/20" : "bg-white/5",
          ...
          {/* Col droite : leaderboard (carte glass) */}
          <aside className="min-w-0">
            <div className="rounded-2xl border border-white/10 bg-white/8 backdrop-blur-md p-4 shadow-[0_12px_30px_rgba(0,0,0,.35)]">
