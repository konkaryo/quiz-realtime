...
const ENERGY_MAX = Number(import.meta.env.VITE_ENERGY_MAX ?? 100); // logique interne, non affichée
...
function Lives({ lives, total }: { lives: number; total: number }) {
  ...
  return <div className="flex justify-start gap-2 mt-3 text-[20px]">{full}{empty}</div>;
}
...
export default function RoomPage() {
  ...
  // secondes restantes (affichage seulement)
  ...
  const resetTimerBar = (dur: number) => {
    ...
    el.style.transform = "scaleX(1)"; // pleine au départ (inverse)
  ...
  useEffect(() => {
    ...
    s.on("final_leaderboard", (p: { leaderboard: LeaderRow[]; displayMs?: number }) => {
      ...
      // (supprimé) pas de message “Fin de partie…”
      stopTimerBar();
    });
    ...
    s.on("game_over", () => {
      ...
      // (supprimé) pas de message “départ imminent…”
      setRevealAnswer(false);
      stopTimerBar();
    });
    ...
    <>
      ...
      <div className="relative z-10 text-white mx-auto w-full max-w-[1200px] px-4 min-h-[calc(100dvh-64px)] pt-6 font-[system-ui]">
        ...
        <div className="grid items-end gap-4 grid-cols-1 md:grid-cols-[minmax(0,1.1fr)_minmax(0,2fr)_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
            <div className="flex items-center justify-end gap-3">
              ...
              <span
                title={`Room ${roomId}`}
                className="inline-flex items-center px-3 py-1 rounded-full border border-white/30 bg-white/10 backdrop-blur-[2px] font-extrabold tracking-wide"
              >
                Room {roomId}
              </span>
            </div>
        ...
        <div className="mt-5 grid gap-6 items-start grid-cols-1 md:grid-cols-[minmax(0,1.05fr)_minmax(0,2fr)_minmax(0,1fr)]">
          {/* *** COLONNE GAUCHE — LEADERBOARD (style maquette) *** */}
          <aside className="min-w-0">
            <div className="rounded-2xl border border-white/10 bg-white/6 backdrop-blur-md p-4 shadow-[0_12px_30px_rgba(0,0,0,.35)]">
              ...
              {leaderboard.length === 0 ? (
                ...
                <ol className="m-0 space-y-3">
                  {leaderboard.map((r, i) => {
                    const baseLi = "flex items-center gap-3";
                    const pill   = "flex-1 flex items-center justify-between rounded-xl px-4 py-2.5 text-[15px] shadow-[0_6px_14px_rgba(0,0,0,.25)]";
                    // Code couleur : sombre pour tous, rang 3 (i===2) en magenta
                    const dark   = "bg-[#0f1420]/90 text-white border border-white/10";
                    const mag3   = "bg-gradient-to-r from-fuchsia-600 to-pink-500 text-white";
                    const pillClass = i === 2 ? mag3 : dark;

                    return (
                      <li key={r.id} className={baseLi}>
                        <span className="w-4 text-right text-xs opacity-80 tabular-nums">{i + 1}</span>
                        <div className={`${pill} ${pillClass}`}>
                          <span className="truncate">{r.name}</span>
                          <span className="tabular-nums ml-4">{r.score}</span>
                        </div>
                      </li>
                    );
                  })}
            ...
            {phase === "final" ? (
              ...
              <div>
                {question && (
                  <>
                    {/* En-tête question */}
                    <div className="rounded-2xl border border-white/15 bg-black/70 px-5 py-4 backdrop-blur-md shadow-[0_12px_24px_rgba(0,0,0,.35)]">
                    ...
                    {/* Image — encadré premium (non déformée) */}
                    {question.img && (() => {
                      ...
                      return (
                        <figure
                          className={[
                            "mt-4 rounded-[22px] p-[2px]",
                            "bg-[linear-gradient(135deg,rgba(255,255,255,.75),rgba(255,255,255,.20))]",
                            "shadow-[0_18px_48px_rgba(0,0,0,.55)]",
                            "will-change-transform transform-gpu",
                          ].join(" ")}
                        >
                          <div className="rounded-[20px] bg-black/85 border border-white/10 overflow-hidden">
                            <div
                              className={[
                                "relative flex items-center justify-center",
                                "bg-black/10",
                                "[mask-image:radial-gradient(120%_120%_at_50%_45%,black_60%,transparent_100%)]",
                                "[-webkit-mask-image:radial-gradient(120%_120%_at_50%_45%,black_60%,transparent_100%)]",
                              ].join(" ")}
                            >
                              <img
                                src={imgSrc}
                    ...
                    {/* Vies (à gauche, juste au-dessus des inputs) */}
          ...
          {/* *** COLONNE DROITE — INFOS (sans messages “départ imminent…”) *** */}
          <aside className="min-w-0">
