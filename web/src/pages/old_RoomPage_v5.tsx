                  ...
                  <>
                    ...
                    {question.img && (() => {
                      ...
                      return (
                        <figure
                          className={[
                            // cadre extérieur dégradé + ombre profonde
                            "mt-4 rounded-[22px] p-[2px]",
                            "bg-[linear-gradient(135deg,rgba(255,255,255,.75),rgba(255,255,255,.20))]",
                            "shadow-[0_18px_48px_rgba(0,0,0,.55)]",
                            // perspective légère pour donner du relief
                            "will-change-transform transform-gpu",
                          ].join(" ")}
                        >
                          {/* passe-partout / mat noir translucide */}
                          <div className="rounded-[20px] bg-black/85 border border-white/10 overflow-hidden">
                            {/* zone visuelle avec vignettage par mask CSS (aucune déformation) */}
                            <div
                              className={[
                                "relative flex items-center justify-center",
                                "bg-black/10",
                                // masque radial pour atténuer les bords (soft vignette)
                            ...
                            >
                              <img
                                ...
                                className={[
                                  ...
                                  // zoom léger au survol (sans jitter)
                                  "transition-transform duration-300 ease-out hover:scale-[1.02]",
                                ].join(" ")}
                                draggable={false}
                              />
                              {/* reflet doux en haut (pure déco) */}
                              <div className="pointer-events-none absolute inset-x-0 top-0 h-16 bg-gradient-to-b from-white/10 to-transparent" />
                            </div>

                            {/* Légende / métadonnées image (optionnelles) */}
                            {(question.theme || typeof question.difficulty === "number") && (
                              <figcaption className="flex items-center justify-between px-4 py-2 text-[12px] text-white/80 border-t border-white/10 bg-white/5">
                                ...
                                {typeof question.difficulty === "number" && (
                                  <span className="inline-flex items-center gap-1 tabular-nums">
                                    ...
                                    <strong>{Math.max(1, Math.min(4, question.difficulty))}</strong>
                                  </span>
                                )}
                              </figcaption>
                            )}
