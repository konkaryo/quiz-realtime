...
const energy_service_1 = require("../domain/player/energy.service");
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("join_game", async (p) => {
            try {
                ...
                const room = await prisma_1.prisma.room.findUnique({ where: { code: roomCode } });
                if (!room)
        ...
        socket.on("submit_answer", async (p, ack) => {
            ...
            const gain = config_1.CFG.AUTO_ENERGY_GAIN + (choice.isCorrect ? config_1.CFG.MC_ANSWER_ENERGY_GAIN : 0);
            const res = await (0, energy_service_1.addEnergy)(prisma_1.prisma, client, gain);
            if (!res.ok)
            ...
            socket.emit("energy_update", {
                energy: res.energy,
                multiplier: (0, energy_service_1.scoreMultiplier)(res.energy),
            });
            const lb = await (0, leaderboard_service_1.buildLeaderboard)(prisma_1.prisma, st.gameId, Array.from(st.pgIds));
        ...
        socket.on("submit_answer_text", async (p, ack) => {
            ...
            // Énergie & score
            const playerEnergy = await (0, energy_service_1.getEnergy)(prisma_1.prisma, client);
            if (!playerEnergy.ok)
                return ack?.({ ok: false, reason: "no-energy" });
            const gain = config_1.CFG.AUTO_ENERGY_GAIN + (correct ? config_1.CFG.TXT_ANSWER_ENERGY_GAIN : 0);
            const res = await (0, energy_service_1.addEnergy)(prisma_1.prisma, client, gain);
            if (!res.ok)
                return ack?.({ ok: false, reason: "no-player" });
            await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (correct) {
                    await tx.playerGame.update({
                        ...
                        data: {
                            energy: res.energy,
                            score: {
                                increment: (0, energy_service_1.scoreMultiplier)(playerEnergy.energy) * config_1.CFG.TXT_ANSWER_POINTS_GAIN,
                            },
                        },
                    });
                    socket.emit("energy_update", {
        ...
        socket.on("request_choices", async () => {
            ...
            if (!client)
                return;
            const res = await (0, energy_service_1.spendEnergy)(prisma_1.prisma, client, config_1.CFG.MC_COST);
            if (!res.ok)
