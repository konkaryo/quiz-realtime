...
exports.buildPlayerSummary = buildPlayerSummary;
exports.buildRoomQuestionStats = buildRoomQuestionStats;
const media_service_1 = require("../media/media.service");
async function buildPlayerSummary(prisma, gameId, playerGameId, orderedQuestions) {
    ...
    if (playerGame.questions?.length) {
        playerGame.questions.forEach((q, idx) => {
            ...
            const img = (0, media_service_1.toImgUrl)(q.img || undefined);
            if (!meta) {
                metas.set(q.id, {
            ...
            if (!meta.img)
                meta.img = img;
            if (!meta.correctLabel && fallbackLabel)
                meta.correctLabel = fallbackLabel;
        });
    }
    let nextIndex = metas.size;
    const ensureMeta = (questionId, fallback) => {
        let meta = metas.get(questionId);
        if (!meta) {
            meta = {
                index: nextIndex++,
                text: fallback.text,
                img: (0, media_service_1.toImgUrl)(fallback.img || undefined),
        ...
        if (!meta.img)
            meta.img = (0, media_service_1.toImgUrl)(fallback.img || undefined);
        if (!meta.correctLabel && fallback.correctLabel)
            meta.correctLabel = fallback.correctLabel;
        return meta;
    };
    ...
    for (const [questionId, meta] of metas) {
        ...
        byQuestion.set(questionId, list.map((item) => ({
            ...item,
            index: meta.index,
            text: meta.text,
            img: meta.img,
...
async function buildRoomQuestionStats(prisma, gameId) {
    const answers = await prisma.answer.findMany({
        where: {
            playerGame: { gameId },
    ...
    const firstCorrectByQPlayer = new Map();
    const playersByQuestion = new Map();
    for (const a of answers) {
        ...
        const mode = a.mode;
        if (!playersByQuestion.has(qid))
            playersByQuestion.set(qid, new Set());
        ...
        if (!m) {
            m = new Map();
            firstCorrectByQPlayer.set(qid, m);
        }
