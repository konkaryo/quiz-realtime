...
import { ArrowDown, ArrowRight, ArrowUpRight, Check, LockKeyhole, Menu, Plus, Send, Star, X } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetchJSON("/auth/me").then((data) => {
      const user = data.user as { guest?: boolean; img?: string | null } | undefined;
  ...
  return (
    <div className="synapz-landing">
      {!landingLoaded && <LoadingScreen hiding={landingLoaderHiding} />}
      <main className={`landing${landingLoaded ? " landing-loaded" : " landing-loading"}`}>
        <section className="hero" aria-labelledby="hero-title">
          ...
          <header className={`site-header${navScrolled ? " site-header-scrolled" : ""}${mobileMenuOpen ? " menu-open" : ""}`}>
            <div className="site-header-inner">
              <div className="site-header-left">
                <a className="brand" href="#top" onClick={() => setMobileMenuOpen(false)}><img src="/landing/logo-dark.png" alt="" /><span>SYNAPZ</span></a>
          ...
          <button className="scroll-hint" onClick={() => roomsRef.current?.scrollIntoView({ behavior: "smooth" })}><span>Jouer</span><ArrowDown size={15} /></button>
        </section>

        <section className="rooms-section" id="salons" ref={roomsRef} aria-labelledby="rooms-title">
          <div className="rooms-heading"><h2 id="rooms-title">Défiez d'autres joueurs.</h2></div>
          ...
          {!loading && !err && rooms.length > 0 && <div className="room-grid">{rooms.map((room) => {
            ...
            return <button className={`room-card difficulty-${level}`} type="button" onClick={() => void openRoom(room.id)} key={room.id}>
              ...
              <div className="room-body">
                <h3>{room.name?.trim() || labels[level - 1]}</h3>
                <div className="room-difficulty" aria-label={`Difficulté : ${level} sur 4`}>{[1, 2, 3, 4].map((star) => <Star key={star} size={13} fill={star <= level ? "currentColor" : "none"} />)}</div>
                <p className="room-description">{roomDescriptions[level - 1]}</p>
              </div>
              <div className="room-players">{count > 0 && <><span className="room-avatars">{visiblePlayers.map((player) => <img src={`${API_BASE}${player.img}`} alt={player.name} key={player.id} />)}</span><strong>{count} joueur{count > 1 ? "s" : ""}</strong></>}</div>
              <span className="room-join">Rejoindre <ArrowRight size={17} /></span>
            </button>;
          })}</div>}
